...
internal class NearMeetupRepositoryImpl(
  ...
  override fun loadNearMeetupList(): Single<List<MeetupEntity>> =
    Single.just(meetupListDataStore.loadMeetupList().events.map { MeetupEntity(it.eventId, it.title, it.eventUrl) })
